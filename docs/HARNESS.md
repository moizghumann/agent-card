...
This repo follows the harness-engineering pattern: keep the active prompt small, keep durable context in the repo, and make correctness executable.
...
- Avoid `examples/*.json`, generated outputs, and broad source sweeps unless the ticket specifically touches output shape or fixtures.
...
- If product behavior changes, update `docs/PRODUCT.md`, `ARCHITECTURE.md`, relevant tests, and examples only when output contracts changed.
